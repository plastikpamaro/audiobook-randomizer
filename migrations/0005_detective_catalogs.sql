...
ALTER TABLE import_sources ADD CONSTRAINT import_sources_kind_check CHECK (
  kind IN ('drei_fragezeichen', 'tkkg', 'van_dusen', 'van_dusen_neue', 'point_whitmark', 'pater_brown', 'csv', 'json', 'rss')
);
ALTER TABLE import_sources ADD CONSTRAINT import_sources_check CHECK (
  (kind IN ('drei_fragezeichen', 'tkkg', 'van_dusen', 'van_dusen_neue', 'point_whitmark', 'pater_brown') AND url IS NULL)
